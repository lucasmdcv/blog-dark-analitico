...
# 2. Requisição via NOVO ROTEADOR (Chat Completions API)
RESPONSE=$(curl -s -X POST "https://router.huggingface.co/hf-inference/v1/chat/completions" \
    -H "Authorization: Bearer $HF_TOKEN" \
    -H "Content-Type: application/json" \
    -d "{
        \"model\": \"$MODEL\",
        \"messages\": [{\"role\": \"user\", \"content\": \"Aja como um analista de elite. Escreva um post analitico para um blog dark sobre: $TEMA. Use markdown, inclua hashtags e uma conclusão técnica.\"}],
        \"max_tokens\": 1200,
        \"temperature\": 0.7
    }")

# Extração segura do JSON via Python para garantir estabilidade
...
# 3. Processamento de Assets e JSON para Netlify
export RESUMO_IA
export TEMA
